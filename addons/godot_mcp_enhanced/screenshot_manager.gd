...
func capture_running_scene_screenshot() -> String:
	...
	# For running scene, we need to capture from the game viewport
	# This requires special handling as the game runs in a separate window/viewport
	...
	# Try to get the running scene window
	# In editor, the game runs in window ID 0 when playing
	var img = DisplayServer.window_get_screenshot(0)
